...
let
  ...
  # Build the Deno npm cache directory
  cache = stdenv.mkDerivation {
    ...
    buildPhase = ''
      ...
      # Extract npm tarballs to cache
      ${lib.concatStringsSep "\n" (lib.mapAttrsToList (name: pkg:
        lib.optionalString (pkg.registryPath != "") ''
          mkdir -p "$out/npm/${pkg.registryPath}"
          tar -xzf ${pkg.src} -C "$out/npm/${pkg.registryPath}" --strip-components=1
        ''
      ) sources)}
    '';
    installPhase = "true";
