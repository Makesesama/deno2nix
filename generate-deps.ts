 ...
 * - NPM packages from npm registry (as tarballs)
 * - JSR packages from jsr.io (as individual files)
...
// Only includes NPM packages - JSR packages are fetched during compile
async function generateDepsNix(lock: DenoLockV5): Promise<string> {
  ...
  // JSR packages will be fetched during deno compile (lock file ensures reproducibility)
  if (lock.npm) {
  ...
  const lines: string[] = [
    ...
    "# NPM packages are pre-fetched; JSR packages are fetched during compile.",
    "",
    "{ stdenv, fetchurl, lib }:",
    "",
    "let",
  ...
  lines.push("  # Build the Deno npm cache directory");
  lines.push("  cache = stdenv.mkDerivation {");
  lines.push('    name = "deno-npm-cache";');
  lines.push("    dontUnpack = true;");
  lines.push("    buildPhase = ''");
  lines.push("      mkdir -p $out/npm");
  lines.push("");
  lines.push("      # Extract npm tarballs to cache");
  lines.push('      ${lib.concatStringsSep "\\n" (lib.mapAttrsToList (name: pkg:');
  lines.push("        lib.optionalString (pkg.registryPath != \"\") ''");
  lines.push('          mkdir -p \"$out/npm/${pkg.registryPath}\"');
  lines.push('          tar -xzf ${pkg.src} -C \"$out/npm/${pkg.registryPath}\" --strip-components=1');
  lines.push("        ''");
  lines.push("      ) sources)}");
  lines.push("    '';");
